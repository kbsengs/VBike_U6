...
if(!objectScript.ODODCOCCDQ){  
Vector3 change = transform.position - oldPos;
if(OCCCCODCOD && oldPos != Vector3.zero && change != Vector3.zero){
int i = 0;
foreach(Transform tr in OCQOCOCQQOs){
...
changed = true;
if(objectScript.ODODCOCCDQ){
objectScript.OOQQCODOCD.specialRoadMaterial = true;
}
}
...
if(objectScript.OOQQCODOCD == null){

ArrayList arr = ODODDCCOQO.OCDCQOOODO(false);
objectScript.OCOQDDODDQ(arr, ODODDCCOQO.OOQOOQODQQ(arr), ODODDCCOQO.OQQDOODOOQ(arr));
...
for(int i = 0; i < objectScript.OCQOCOCQQOs.Length; i++){
if(objectScript.OCQOCOCQQOs[i] == this.gameObject)return true;
}
return false;
}
}
